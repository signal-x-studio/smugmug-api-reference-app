...
import { Photo, FilterState, FilterCombination } from '../types';
...
export const FilterPanel: React.FC<FilterPanelProps> = React.memo(({
  ...
  }, [currentFilters, setCurrentFilters, debouncedFilterChange]);
  ...
  const handleClearAll = useCallback(() => {
    ...
    clearAllFilters();
    // Clear action should be immediate, not debounced
    onFilterChange(clearedFilters);
  }, [clearAllFilters, onFilterChange]);

  const handleToggleCombination = useCallback(() => {
    toggleCombinationMode();
    // Notify parent about combination change with current filters
    onFilterChange(currentFilters, combMode === 'AND' ? 'OR' : 'AND');
  }, [toggleCombinationMode, onFilterChange, currentFilters, combMode]);

  // Define filter content that can be reused
  function FilterContent(): React.ReactElement {
    return (
      <>
        <FilterHeader
          combMode={combMode}
...
// Export types and components for compatibility with existing tests
export type { FilterState, FilterCombination };

// Export the actual implemented components

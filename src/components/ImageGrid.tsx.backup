...
export const ImageGrid: React.FC<ImageGridProps> = ({ 
  ...
  const isSyncing = isLoading === 'syncing';
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      // Optional: Clear the data transfer to prevent browser from opening the file
  ...
  if (isLoading === 'photos') {
      return (
        <div>
            <h2 className="text-3xl font-bold text-white mb-1">Loading photos in <span className="text-cyan-400">{album.name}</span>...</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-4">
              {[...Array(album.imageCount || 6)].map((_, i) => (
  ...
  return (
    ...
    >
        <div className="flex justify-between items-start mb-4 flex-wrap gap-4">
             <div>
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={onOpenAlbumSettings} className="p-2 rounded-full text-slate-400 hover:bg-slate-700 hover:text-white transition" aria-label="Open Album AI Settings">
            ...
            <div className="flex items-center gap-2">
                <button
                    ...
                    className={`flex items-center gap-2 font-semibold px-4 py-2 rounded-lg transition ${isSelectionMode ? 'bg-blue-800 text-white' : 'bg-slate-700 hover:bg-slate-600 text-white'}`}
                ...
                >
                    {isSyncing ? <IconSync className="w-5 h-5 animate-spin" /> : <IconSync className="w-5 h-5" />}
                    Sync
                </button>
                 <button 
        ...
        <div className="flex justify-between items-center mb-6 flex-wrap gap-4 border-b border-slate-700 pb-4">
            <div className="flex items-center bg-slate-800 rounded-lg p-1 border border-slate-700">
                ...
                >
                    ...
                    <span className={`flex items-center justify-center w-5 h-5 text-xs font-bold rounded-full ${
                        viewMode === 'review' ? 'bg-black text-yellow-400' : 'bg-yellow-500 text-black'
                    }`}>
                        {reviewCount}
                    </span>
                    )}
            ...
            {viewMode === 'review' && photos.length > 0 && (
            <div className="flex items-center gap-2">
                <button onClick={onRejectAll} className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold rounded-md bg-red-900/80 hover:bg-red-800 text-red-200 transition">
                    <IconTrash className="w-4 h-4" /> Reject All
                </button>
                <button onClick={onApproveAll} className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold rounded-md bg-green-600 hover:bg-green-700 text-white transition">
